...
> **2026-07-06:** hard prose-numeric backstop for MIR-less `whySimilar`. The prompt already forbids stating a tempo/BPM or key as shared evidence when `criteria` is empty (there is no validated acoustic comparison to back it), but that was prompt-only. Added a validation-layer backstop — `_prose_states_ungrounded_metric(prose)` (narrow regexes: BPM-anchored tempo; a key needs an uppercase note letter + required `major`/`minor`, or the phrase "key of"; a stray English "key"/"a minor role" does not trip it) — wired into `generate_narrative` **only** for `mode == "whySimilar" and not context.criteria`, returning `NarrativeUnavailable(reason="prose-ungrounded-metric")` (a new, distinct reason; not cached). Deliberately gated: craft modes legitimately state the *upload's own* detected "129 BPM in A minor", and a criteria-present `whySimilar` has a validated citation — both untouched (proven by control tests). So an ungrounded number can never render on a MIR-less match. Golden case `prose_ungrounded_bpm_on_mirless_whysimilar` added; all 5 baselines at 1.0. Verified live: the grounded "Champ In Blue" whySimilar prose (no BPM/key) still returns kind `narrative` — no false rejection.
