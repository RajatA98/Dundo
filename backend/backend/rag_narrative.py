...
SYSTEM_PROMPTS: dict[NarrativeMode, str] = {
    "whySimilar": (
        ...
        "by name and where they're based (when that's in artistKnowledge — it's a real, "
        "human detail worth opening on), then connect their sound to the upload. Be warm "
        "and human — vary your phrasing and NEVER use stiff, formulaic openings like 'The "
        "matched artist resonates with your creation due to their shared genres of…' or "
        "'The shared genres of X and Y create a…'. "
        "Ground the explanation in the supplied evidence: the tempo, "
        ...
        "a genre, mood, or instrument. "
        "MIR-LESS MATCHES — READ CAREFULLY. Very often the `criteria` array is EMPTY. That "
        "is a normal, well-supported match, not a weak one: you still have the shared "
        "genre/mood descriptors in `sharedDescriptors`, the artist facts in "
        "`artistKnowledge`, and the matched moment (`matchWindow`). When `criteria` is "
        "empty, write with confidence from those three — name the shared vibe (e.g. their "
        "pop-jazz warmth meeting yours), lead with the artist and their location, and you "
        "MAY note where it lands most, e.g. 'it resonates most around the 1:20 mark' using "
        "`matchWindow` (seconds → m:ss). But when `criteria` is empty there is NO validated "
        "acoustic comparison, so you MUST NOT state any specific tempo/BPM number, key, or "
        "other numeric acoustic value as a point of resemblance — those are unverified and "
        "reading them as shared evidence is a fabrication. `queryDescriptors` describes the "
        "UPLOAD only; do not present its tempo/key as something the two tracks share. "
        "CITATIONS RULE (hard, applies at every level): a citation object asserts a "
        "validated MIR criterion. If — and only if — the `criteria` array is non-empty may "
        "you emit citations, one per criterion you actually reference. If `criteria` is "
        "EMPTY, `citations` MUST be the empty list []. Never emit a citation object in that "
        "case — not a stray one, not an empty `timestampRange`, not a placeholder. The "
        "prose still stands on its own, grounded in tags + artist facts + the matched moment. "
        + _ARTIST_KNOWLEDGE_RULE
